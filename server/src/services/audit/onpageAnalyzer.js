...
export function analyzeOnPage(pages) {
  ...
  // ── 6. Target Keyword Optimization ──────────────────────────────
  if (targetKeyword) {
    const kwLower = targetKeyword.toLowerCase()
    const missingKwTitle = []
    const missingKwH1 = []
    ...
    pages.forEach(p => {
      ...
      if (!inTitle) missingKwTitle.push(p.url)
      if (!inH1) missingKwH1.push(p.url)
      if (density > 3.5) keywordStuffing.push({ url: p.url, count, density })
    })

    if (missingKwTitle.length > 0) {
      issues.push(
        createIssue(
          'onpage',
          'MEDIUM',
          `Target Keyword Missing from Title Tag ("${targetKeyword}")`,
          `Including your primary focus keyword near the front of the title tag correlates strongly with organic rankings.`,
          `Incorporate "${targetKeyword}" naturally into the title tag.`,
          { affectedPages: missingKwTitle }
        )
      )
    }

    if (missingKwH1.length > 0) {
      issues.push(
        createIssue(
          'onpage',
          'MEDIUM',
          `Target Keyword Missing from Primary H1 ("${targetKeyword}")`,
          `Aligning the H1 heading with the primary target keyword reassures users and search engines of topical relevance.`,
          `Include "${targetKeyword}" in the page H1 heading.`,
          { affectedPages: missingKwH1 }
        )
      )
    }
    ...
    if (keywordStuffing.length > 0) {
      issues.push(
        createIssue(
          'content',
          'HIGH',
          `High Keyword Density / Potential Over-Optimization ("${targetKeyword}")`,
          `Keyword density over 3.5% can trigger algorithmic over-optimization penalties under Google Helpful Content guidelines.`,
          `Reduce repetitive keyword usage and incorporate natural semantic variations and synonyms.`,
          {
            affectedPages: keywordStuffing.map(k => k.url),
            affectedItems: keywordStuffing.map(k => ({
              url: k.url,
              evidence: `Appears ${k.count} times (${k.density}% density)`,
            })),
          }
        )
      )
