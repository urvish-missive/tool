  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4" onClick={handleBackdropClick}>
      <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden animate-fade-in">
        {/* Gradient header */}
        <div className="relative h-2" style={{ background: 'linear-gradient(77deg, #0C81F3 32%, #EB8988 100%)' }} />
        ...
        <div className="p-6 sm:p-8">
          ...
          <div className="text-center mb-6">
            <span className="inline-block px-3 py-1 bg-gradient-to-r from-[#0C81F3] to-[#EB8988] text-white text-xs font-bold rounded-full mb-3 tracking-wide uppercase">Free Tool</span>
            <h2 className="text-xl font-bold text-gray-900 mb-1">{title || 'Get Your Free Results'}</h2>
            <p className="text-sm text-gray-500">{subtitle || 'Enter your details to unlock this free SEO tool'}</p>
          </div>
              ...
              {fields.map(f => (
                <div key={f.name} className={!f.col ? 'sm:col-span-2' : ''}>
                  ...
                  <input
                    ...
                    placeholder={f.type === 'email' ? 'you@company.com' : f.type === 'tel' ? '+1 234 567 890' : ''}
                  />
                </div>
              ))}
            </div>
