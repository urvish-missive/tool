...
function FinancialTrajectoryChart({ monthlyData, currency, breakEvenMonth }) {
  ...
  const revPath = series
    .map((s, i) => `${i === 0 ? 'M' : 'L'} ${getX(i)} ${getY(s.cumRev)}`)
    .join(' ')
  const invPath = series
    .map((s, i) => `${i === 0 ? 'M' : 'L'} ${getX(i)} ${getY(s.cumInv)}`)
    .join(' ')
    ...
    <div className="relative bg-slate-900 text-white p-6 rounded-2xl border border-slate-800 shadow-xl overflow-hidden">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Cumulative Revenue vs Cumulative Investment over campaign horizon
          </p>
        </div>
      ...
      <div className="overflow-x-auto">
        <svg
          ...
          className="w-full max-w-2xl mx-auto overflow-visible font-sans"
        >
          <defs>
            ...
            return (
              ...
              >
                ...
                <circle
                  ...
                  cy={yRev}
                  r={isBreakEven ? 6 : 4}
                  fill={isBreakEven ? '#10b981' : '#34d399'}
                  stroke="#0f172a"
                  strokeWidth="2"
                />
                <text
      ...
      {hoveredIdx !== null && series[hoveredIdx] && (
        <div className="mt-4 p-3 rounded-xl bg-slate-800/90 border border-slate-700 flex flex-wrap items-center justify-between text-xs gap-3">
          <span className="font-bold text-slate-200">
            Month {series[hoveredIdx].month} Projections:
          </span>
          <div className="flex items-center gap-4">
            <span className="text-emerald-400">
              Cum. Revenue: <strong>{fmt(series[hoveredIdx].cumRev, currency)}</strong>
            </span>
            <span className="text-blue-400">
              Cum. Invested: <strong>{fmt(series[hoveredIdx].cumInv, currency)}</strong>
            </span>
            <span
              className={
                series[hoveredIdx].net >= 0
      ...
      <section className="relative overflow-hidden !pt-36 py-16 sm:py-20 lg:py-24">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 text-center">
          ...
          <p className="mt-3 text-lg text-gray-600 max-w-2xl mx-auto leading-relaxed">
            Model organic growth scenarios, break-even timelines, and generate an investor-ready
            business case to justify SEO investment.
          </p>
        </div>
      </section>
          ...
          <div id="roi-results" className="space-y-6 animate-fade-in">
                ...
                {aiReport.phasedRoadmap?.length > 0 && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6">
                    {aiReport.phasedRoadmap.map((p, i) => (
                      <div
                        key={i}
                        className="bg-white/5 rounded-2xl p-5 border border-white/10 space-y-2"
                      >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                {
                  name: 'Conservative',
                  data: results?.conservative,
                  color: 'text-blue-600',
                  border: 'border-blue-500',
                },
                {
                  name: 'Moderate',
                  data: results?.moderate,
                  color: 'text-emerald-600',
                  border: 'border-emerald-500',
                },
                {
                  name: 'Aggressive',
                  data: results?.aggressive,
                ...
                return (
                  <div
                    ...
                    className={`cursor-pointer bg-white rounded-2xl p-6 border-2 transition-all shadow-sm hover:shadow-md ${
                      isSelected
                        ? `${border} bg-emerald-50/20 shadow-md ring-2 ring-emerald-500/20`
                        : 'border-slate-200'
                    }`}
                  >
                    ...
                    <div className={`text-3xl sm:text-4xl font-black ${color} mb-1`}>{roiVal}%</div>
                    ...
                    <div className="mt-4 pt-4 border-t border-slate-100 space-y-1.5 text-xs">
                      <div className="flex justify-between">
                        ...
                        <strong className="text-slate-900">
                          {fmt(data?.summary?.additionalRevenue, currency)}
                        </strong>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-500">Net Profit:</span>
                        <strong className="text-emerald-700">
                          {fmt(data?.summary?.netReturn, currency)}
                        </strong>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-500">Total Cost:</span>
                        <strong className="text-slate-700">
                          {fmt(data?.summary?.totalInvestment, currency)}
                        </strong>
                      </div>
                    </div>
                  </div>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm space-y-4">
                ...
                <div className="space-y-3">
                  {aiReport?.largest_levers?.map((lever, i) => (
                    <div
                      key={i}
                      className="p-3 bg-slate-50 rounded-xl border border-slate-200 text-xs text-slate-800"
                    >
