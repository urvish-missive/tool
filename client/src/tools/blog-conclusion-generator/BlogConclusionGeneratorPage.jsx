...
import {
  blogConclusionSchema,
  parseBlogConclusionForm,
} from '../../schemas/blogConclusion.schema'
...
export default function BlogConclusionGeneratorPage() {
  ...
  const handleDownloadMarkdown = () => {
    if (!dataResult?.conclusions?.length) return
    const activeToneLabel = TONES.find((t) => t.id === (dataResult.tone || watch('tone')))?.label || (dataResult.tone || watch('tone'))
    const content = `# High-Converting Blog Conclusions: ${dataResult.topic}\n\n*Tone: ${activeToneLabel}*\n*Generated by Missive Digital Blog Conclusion Generator on ${new Date().toLocaleDateString()}*\n\n` +
    ...
    link.href = url
    link.download = `blog-conclusions-${dataResult.topic.toLowerCase().replace(/[^a-z0-9]+/g, '-').slice(0, 40)}.md`
          ...
          <div className="bg-white rounded-2xl sm:rounded-3xl shadow-lg shadow-slate-200/40 border border-slate-200 p-4 sm:p-6 lg:p-7 mb-8 transition-all">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 p-3 sm:p-3.5 rounded-2xl bg-gradient-to-r from-blue-50/90 via-indigo-50/30 to-slate-50 border border-blue-100/90 mb-4 sm:mb-5">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <span className="text-xs font-bold text-slate-800 block leading-tight">Try a Sample Scenario:</span>
                  <span className="text-[11px] text-slate-500">1-click fill with realistic B2B SEO and content data</span>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit(onFormValid, onFormInvalid)} className="space-y-4 sm:space-y-5">
              {/* Blog Topic Input */}
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label htmlFor="conclusion-topic-input" className="block text-xs sm:text-sm font-bold text-slate-800">
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label htmlFor="conclusion-intro-input" className="block text-xs sm:text-sm font-bold text-slate-800 flex items-center gap-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      type="button"
                      onClick={() => setValue('intro', SAMPLE_PRESETS[0].intro, { shouldValidate: true })}
                ...
                <p className="mt-1 text-[11px] text-slate-400">
                  Optional but recommended: Great conclusions deliver on the promise established in the article's opening.
                </p>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label htmlFor="conclusion-takeaways-input" className="block text-xs sm:text-sm font-bold text-slate-800 flex items-center gap-1.5">
                    <CheckCircle2 className="w-3.5 h-3.5 text-slate-500" />
                    <span>Key Takeaways / Highlights <span className="text-xs font-normal text-slate-400">(Optional)</span></span>
                  </label>
                  <button
                    type="button"
                    onClick={() => setValue('keyTakeaways', SAMPLE_PRESETS[0].keyTakeaways, { shouldValidate: true })}
                    className="text-[11px] text-[#0C81F3] hover:underline font-semibold cursor-pointer"
                  >
                    Sample Takeaways
                  </button>
                ...
                <p className="mt-1 text-[11px] text-slate-400">
                  Optional: Key takeaways covered in your article body so the generator can synthesize them into the final CTA bridge.
                </p>
              </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {CTA_OPTIONS.map((opt) => {
                    ...
                    return (
                      ...
                      >
                        <Icon className={`w-3.5 h-3.5 shrink-0 ${isSelected ? 'text-[#0C81F3]' : 'text-slate-400'}`} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
                {/* Tone Selector */}
                <div>
                  <label htmlFor="conclusion-tone" className="block text-xs sm:text-sm font-bold text-slate-800 mb-1">
                    Tone of Voice
                  </label>
                  <select
                    id="conclusion-tone"
                ...
                {/* Target Audience */}
                <div>
                  <label htmlFor="conclusion-audience" className="block text-xs sm:text-sm font-bold text-slate-800 mb-1">
                  ...
                  <div className="grid grid-cols-3 gap-1.5">
                    {[
                      ...
                      >
                        ...
                        <div className="text-[9px] text-slate-400 font-normal leading-none mt-0.5">{sub}</div>
                        {badge && countValue !== n && (
          ...
          <div id="conclusion-results" className="space-y-6 animate-fade-in pt-1">
            ...
            <div className="bg-white rounded-2xl sm:rounded-3xl p-4 sm:p-5 border border-slate-200 shadow-xs flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Tone: <strong className="text-slate-700 capitalize">{TONES.find((t) => t.id === (dataResult.tone || watch('tone')))?.label || (dataResult.tone || watch('tone'))}</strong>
                </p>
              </div>
            ...
            <div className="flex items-center justify-between border-b border-slate-200 pb-2 overflow-x-auto no-scrollbar gap-2">
              ...
              <span className="text-xs text-slate-400 font-medium shrink-0 hidden sm:inline">
                Showing {filteredConclusions.length} result{filteredConclusions.length !== 1 ? 's' : ''}
              </span>
            </div>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                      ...
                      >
                        {isH2Copied ? <Check className="w-3 h-3 text-emerald-600" /> : <Copy className="w-3 h-3" />}
                    ...
                    <div className="p-3 sm:p-3.5 rounded-xl bg-gradient-to-r from-blue-50/70 via-slate-50 to-pink-50/40 border border-blue-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
                      ...
                      <button
                        type="button"
                        onClick={() => handleCopyCta(item.id, `${item.ctaPrompt}\n[${item.ctaButtonText}]`)}
                        className="self-start sm:self-auto px-2.5 py-1 rounded-lg bg-white border border-slate-200 hover:bg-slate-100 text-slate-700 text-xs font-semibold flex items-center gap-1 transition-colors cursor-pointer shrink-0 shadow-2xs whitespace-nowrap"
                      >
                        {isCtaCopied ? <Check className="w-3 h-3 text-emerald-600" /> : <Copy className="w-3 h-3" />}
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-2 border-t border-slate-100">
                      <div className="text-[11px] text-slate-500 italic max-w-xl">
                        <span className="font-semibold text-slate-700 not-italic">Why it works: </span>
                        {item.whyItWorks}
                      </div>
