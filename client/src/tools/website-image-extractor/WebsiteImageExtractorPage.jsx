...
export default function WebsiteImageExtractorPage() {
  ...
  const [downloadFormat, setDownloadFormat] = useState('original') // 'original' | 'png' | 'jpg' | 'webp' | 'svg'
  ...
  // Format options for image downloading
  const FORMAT_OPTIONS = [
    { value: 'original', label: 'Original Format (Auto)' },
  ...
  // Convert image Blob to target format (PNG, JPG, WEBP, SVG) via HTML5 Canvas or SVG wrapping
  const convertImageBlob = async (sourceBlob, targetFormat, originalExt, dimensions = {}) => {
    const normFormat = (targetFormat || 'original').toLowerCase()
    if (normFormat === 'original') {
      return { blob: sourceBlob, extension: originalExt }
    }

    // SVG handling
    if (normFormat === 'svg') {
      if (sourceBlob.type?.includes('svg') || originalExt === '.svg') {
        return { blob: sourceBlob, extension: '.svg' }
      }
      // Wrap raster into an SVG image element container
      ...
      const svgCode = `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}"><image width="${w}" height="${h}" href="${dataUrl}"/></svg>`
      return {
        blob: new Blob([svgCode], { type: 'image/svg+xml;charset=utf-8' }),
        extension: '.svg',
      }
    }
    ...
    const targetMime = mimeMap[normFormat] || 'image/png'
    const targetExt = normFormat === 'jpeg' || normFormat === 'jpg' ? '.jpg' : `.${normFormat}`

    if (sourceBlob.type === targetMime) {
      return { blob: sourceBlob, extension: targetExt }
    }

    // Load into Image object and draw onto Canvas
    ...
    await new Promise((resolve, reject) => {
      ...
      imgObj.onerror = () => reject(new Error('Failed to load image on canvas for conversion'))
      imgObj.src = sourceObjUrl
    })

    const canvas = document.createElement('canvas')
    canvas.width = imgObj.naturalWidth || imgObj.width || 400
    canvas.height = imgObj.naturalHeight || imgObj.height || 300
    const ctx = canvas.getContext('2d')

    // If converting to JPG, fill white background to avoid transparent pixels becoming black
    ...
    const convertedBlob = await new Promise((resolve) => {
      canvas.toBlob(
        (b) => resolve(b),
        targetMime,
        targetMime === 'image/jpeg' ? 0.92 : 0.9
      )
    })

    return { blob: convertedBlob || sourceBlob, extension: targetExt }
  }

  // Reliable Image Download (Blob-based with format conversion)
  const downloadSingleImage = async (img, formatOverride = null) => {
    const targetFormat = formatOverride || downloadFormat
    ...
    try {
      ...
      const originalExt = extMap[fmt] || (img.url?.toLowerCase().includes('.svg') ? '.svg' : '.png')
      ...
      if (img.url.startsWith('data:')) {
        ...
        const downloadEndpoint = getApiUrl(
          `/image-extractor/download?url=${encodeURIComponent(img.url)}&filename=${encodeURIComponent(img.filename || 'image')}`
        )
        ...
        sourceBlob = await resp.blob()
        if (sourceBlob.type && sourceBlob.type.includes('text/html')) {
          throw new Error('Received an HTML page instead of an image')
        }
      }

      // Convert image to chosen format
      const { blob: finalBlob, extension: finalExt } = await convertImageBlob(
        sourceBlob,
        targetFormat,
        originalExt,
      ...
      let baseName = (img.filename || 'image')
        .replace(/\.[a-zA-Z0-9]+$/, '')
        .replace(/[^a-zA-Z0-9._-]/g, '_')
      if (!baseName || baseName === '_') baseName = 'image'
      ...
      console.warn('Proxy download or conversion error, opening directly in new tab:', err.message)
                      ...
                      {batchProgress ? (
                        ...
                        <>
                          ...
                          <span>
                            ...
                            {downloadFormat !== 'original' && ` (${downloadFormat.toUpperCase()})`}
                          </span>
                        </>
                      )}
                    </button>
                        ...
                        <div className="pt-3 border-t border-slate-100 flex items-center justify-between gap-2">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            >
                              ...
                              <span>{isDownloading ? 'Saving...' : downloadFormat === 'original' ? 'Save' : `Save ${downloadFormat.toUpperCase()}`}</span>
                            </button>
                          </div>
                        </div>
                      </div>
                            ...
                            <td className="px-4 py-3 text-right">
                              <div className="inline-flex items-center gap-2">
                                ...
                                >
                                  ...
                                  <span>{isDownloading ? 'Saving...' : downloadFormat === 'original' ? 'Download' : `Download ${downloadFormat.toUpperCase()}`}</span>
                                </button>
                              </div>
                            </td>
                          </tr>
          ...
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            <div className="bg-white rounded-3xl max-w-3xl w-full overflow-hidden shadow-2xl space-y-4">
              ...
              <div className="p-5 border-t border-slate-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-2">
                  <div className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 rounded-full px-3 py-1.5 text-xs font-bold text-slate-700">
                    <span className="text-[10px] text-slate-400 uppercase tracking-wider">Format:</span>
                    <select
                      value={downloadFormat}
                      onChange={(e) => setDownloadFormat(e.target.value)}
                      className="bg-transparent focus:outline-none cursor-pointer pr-1 font-semibold"
                    >
                      {FORMAT_OPTIONS.map((f) => (
                        <option key={f.value} value={f.value}>
                          {f.label}
