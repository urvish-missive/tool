...
export default function AdminLayout() {
  ...
  return (
    <div className="h-screen bg-gray-50 flex overflow-hidden">
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-gray-200 flex flex-col transform transition-transform duration-200 lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="shrink-0 p-4 border-t border-gray-100">
          <div className="flex gap-2">
            <Link to="/" className="flex-1 text-center text-xs text-gray-500 hover:text-gray-900 py-2 rounded-lg hover:bg-gray-50 transition-colors">
              View Site
            </Link>
            <button onClick={logout} className="flex-1 text-xs text-red-500 hover:text-red-700 py-2 rounded-lg hover:bg-red-50 transition-colors">
              Logout
            </button>
          </div>
        </div>
