...
export default function ContentAnalyzerPage() {
  ...
  const {
    ...
    showPopup,
    setShowPopup,
    handlePopupSubmit: onPopupSubmit,
    handlePopupClose,
    triggerPopup,
  } = useLeadPopup('content-analyzer')

  // Admin-controlled field visibility
  const { isFieldEnabled } = useToolFields('content-analyzer')

  // RTK Query mutation
  const [analyzeContent, { isLoading, isError, error, data, reset: resetMutation }] =
  ...
  const handleSubmit = async (e) => {
    ...
    if (isFieldEnabled('content')) {
      ...
      if (content.trim().length < 100) {
        setValidationError(
          `Content must be at least 100 characters. Currently ${content.trim().length}.`
        )
        return
      }
  ...
  const errorMessage =
    error?.data?.error ||
    (isError ? "We couldn't complete the analysis right now. Please try again in a moment." : '')
      ...
      <section className="relative overflow-hidden !pt-36 py-16 sm:py-20 lg:py-24">
        ...
        <div className="relative max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <p className="mt-5 text-lg text-gray-600 max-w-2xl mx-auto leading-relaxed">
            Analyze your content and discover how to improve its SEO, readability, structure, search
            intent and overall content quality.
          </p>
        </div>
      </section>
        ...
        >
          {status === 'idle' && (
            <form
              ...
              className="bg-white rounded-2xl border border-gray-200 shadow-lg shadow-gray-200/50 p-6 sm:p-8 space-y-6"
            >
              {/* Content */}
              <div>
          ...
          {/* Loading */}
          {status === 'loading' && <LoadingProgress currentStep={loadingStep} />}
          ...
          {status === 'error' && (
            <div className="bg-white rounded-2xl border border-red-200 shadow-sm p-8 text-center">
              ...
              <button
                onClick={handleReset}
                className="mt-4 px-6 py-2.5 rounded-lg bg-gray-100 text-sm font-medium text-gray-700 hover:bg-gray-200 transition-colors cursor-pointer"
              >
                Try Again
              </button>
            </div>
