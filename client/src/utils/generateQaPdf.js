...
export function generateQaPdf(report, meta = {}) {
  ...
  // Header Title
  y = 15
  text(doc, "Himani Kankaria's Content QA Checklist", M, y, {
    size: 18,
    style: 'bold',
    color: C.dark,
  })
  y += 6
  text(doc, 'that helps you QA every content written for your brand the way Himani does.', M, y, {
    size: 8.5,
    color: C.primary,
  })
  y += 4
  text(doc, '12 Pillars \u2022 34 Quality & AI Checks \u2022 Deep Editorial Assessment', M, y, {
    size: 7.5,
    color: C.gray,
  })
  ...
  text(doc, 'The Himani Score', M + 24, y + 37, {
    size: 7.5,
    style: 'bold',
    color: C.dark,
    align: 'center',
  ...
  if (meta.title)
    text(doc, `Title: ${meta.title.substring(0, 35)}`, M + 46, y + 31, { size: 7.5, color: C.mid })
  if (meta.keyword)
    text(doc, `Target Keyword: ${meta.keyword}`, M + 46, y + 37, { size: 7.5, color: C.mid })
  ...
  text(doc, `Em Dashes: ${quick.emDashesCount || 0} (Goal: 0)`, qx + 4, y + 13.5, {
    size: 7,
    style: 'bold',
    color: quick.emDashesCount === 0 ? C.green : C.red,
  })

  doc.setFillColor(...(quick.aiPhrasesCount === 0 ? C.greenBg : C.yellowBg))
  doc.roundedRect(qx, y + 18, 55, 8, 2, 2, 'F')
  text(doc, `AI Clichés: ${quick.aiPhrasesCount || 0} detected`, qx + 4, y + 23.5, {
    size: 7,
    style: 'bold',
    color: quick.aiPhrasesCount === 0 ? C.green : C.yellow,
  })

  doc.setFillColor(...C.lightGray)
  doc.roundedRect(qx, y + 28, 55, 8, 2, 2, 'F')
  text(doc, `Flesch Reading Ease: ${quick.fleschScore || 0}/100`, qx + 4, y + 33.5, {
    size: 7,
    color: C.mid,
  })
  ...
  if (ai?.summary) {
    ...
    text(doc, "Himani's Executive Assessment", M + 4, y + 5, {
      size: 8,
      style: 'bold',
      color: C.primary,
    })
  ...
  for (const [catId, catDef] of Object.entries(categories)) {
    ...
    text(doc, `${catDef.number}. ${catDef.label}`, M + 4, y + 5.5, {
      size: 8.5,
      style: 'bold',
      color: C.dark,
    })
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    text(doc, "Himani Kankaria's Content QA Checklist \u2022 Missive Digital", M, H - 1.5, {
      size: 6,
      color: C.white,
    })
