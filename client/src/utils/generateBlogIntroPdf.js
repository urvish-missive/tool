...
/**
 * Blog Intro Generator PDF — a list of generated introduction variations
 * grouped by funnel stage, in Missive's branded report style.
 */
export function generateBlogIntroPdf(result) {
  ...
  y = 27
  text(doc, 'Blog Introduction Variations', M, y, { size: 17, style: 'bold', color: C.dark })
  y += 5.8
  text(doc, result.summary?.topic || 'Untitled Topic', M, y, { size: 10, color: C.primary, style: 'bold' })
  y += 6

  const introductions = result.introductions || []
  doc.setFillColor(...C.lightGray)
  doc.setDrawColor(...C.border)
  doc.roundedRect(M, y, CW, 12, 2.5, 2.5, 'FD')
  text(doc, `Tone: ${result.summary?.tone || 'Conversational'}`, M + 4, y + 7.2, { size: 8, color: C.mid })
  text(doc, `${introductions.length} variation(s) generated`, M + CW - 4, y + 7.2, {
  ...
  for (const intro of introductions) {
    doc.setFont('helvetica', 'normal')
    doc.setFontSize(8.5)
    const hookLines = intro.hookLine ? doc.splitTextToSize(`"${intro.hookLine}"`, CW - 12) : []
    const bodyLines = intro.fullIntro ? doc.splitTextToSize(intro.fullIntro, CW - 12) : []
    const transitionLines = intro.transition ? doc.splitTextToSize(intro.transition, CW - 12) : []

    const blockHeight =
      6 + hookLines.length * 4.6 + 3 + bodyLines.length * 4.2 + (bodyLines.length ? 2 : 0) +
      transitionLines.length * 4.2 + 8
    ...
    const stageLabel = STAGE_LABELS[intro.funnelStage] || (intro.funnelStage || '').toUpperCase()
    ...
    text(doc, intro.hookFormula || '', M + CW - 4, y, { size: 6.5, color: C.gray, align: 'right' })
    y += 6
